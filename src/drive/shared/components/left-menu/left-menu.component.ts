...
export class ZDriveSharedLeftMenuComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.driveStorage.currentFolder$
      .pipe(filter(f => !!f), takeUntil(this.destroySubject))
  ...
  onOpenFolderUpload() {
    this.fileDriveUploadService.open({ folderOnly: true, accept: "*" });
  }

  onFolder() {
    this.driveServive.modalEvent({
      action: "drive:folder:edit",
      payload: { mode: "add" }
    });
  }
  ...
  loadMenu(event: any) {
    ...
    if (items && items.length === 0) {
      if (expanded) {
        this.folderService.get(id).subscribe((res: any) => {
          this.commonEventService.broadcast({
            action: "update",
            channel: "driveLeftMenu",
            payload: res.data
