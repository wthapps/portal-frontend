...
export class MPhotosService {
  ...
  getData(nextLink?: any): Observable<any> {
    const link = nextLink ? nextLink : 'media/media/index_combine';
  ...
  sort(sort: any) {
    ...
    return this.getData(`media/media/index_combine?sort=${sort.orderBy}&sort_name=${sort.sortBy}`);
  }
  ...
  delete(items?: any) {
    return this.api.post(`media/media/delete`, { objects: items }).pipe(
      tap(() => {
        const newData = this.dataSubject.getValue();
        // items.map((v: Media) => {
        //   newData.filter(item => item.id !== v.id);
        // });
