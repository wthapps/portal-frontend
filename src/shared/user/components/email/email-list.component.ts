  ...
  initItem(item?: any) {
    if (item) {
      return this.fb.group({
        ...
        value: [item.value, Validators.compose([Validators.maxLength(100), CustomValidator.emailFormat])]
      });
    } else {
      return this.fb.group({
        category: ['work'],
        value: ['', Validators.compose([Validators.maxLength(100), CustomValidator.emailFormat])]
      });
    }
  }
