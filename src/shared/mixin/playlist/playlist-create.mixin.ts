...
export class PlaylistCreateMixin {
  ...
  openCreatePlaylistModal(selectedObjects: any) {
    ...
    selectedObjects = selectedObjects.filter(s => (s.model === 'Media::Video' || s.object_type === 'video'));
  ...
  onDonePlaylist(e: any) {
    this.apiBaseService.post(`media/playlists`, { name: e.parents[0].name,
       description: e.parents[0].description, videos: e.children.map(el => el.id) }).subscribe(res => {
