...
export const SO_PROFILE_SETTING_PRIVACY_UPDATE_DONE =
  '[SOCIAL] SO PROFILE SETTING PRIVACY UPDATE DONE';

export const POSTS_COUNT_LOAD = '[SOCIAL] NEW SOCIAL POSTS COUNT LOAD';
export const POSTS_COUNT_LOAD_DONE =
  '[SOCIAL] NEW SOCIAL POSTS COUNT LOAD DONE';
...
export function appReducer(state = socialInitialState, action: any): AppState {
  switch (action.type) {
    ...
    case SHORTCUT_ADD_MULTI_DONE: {
      const shortcuts = { ...state.shortcuts, ...action.payload };
      return { ...state, shortcuts: shortcuts };
    }
    ...
    case SO_PROFILE_SETTING_PRIVACY_UPDATE_DONE: {
      const soProfile = { ...state.soProfile };
...
function compareBy(
  ...
  if (objA[field] > objB[field]) return 1 * o;
  if (objA[field] < objB[field]) return -1 * o;
